// Mevcut EvaluationUpsell satırlarına budgetConstraint alanını doldurur.
//
// Yalnızca kuralın sonucu değiştirebileceği satırlara dokunur:
//   (stemCell = SUNULMADI VEYA premium = SUNULMADI)  AND  budgetConstraint IS NULL
// Bütçe kuralı sadece CEZA kaldırıyor (bkz. okr.ts effectiveStatus); ikisi de
// SUNULDU/NA olan satırlarda hiçbir şeyi değiştirmediği için onlar atlanır.
//
// Yeniden çalıştırılabilir: dolu satırlara dokunmaz, yarıda kesilirse kaldığı
// yerden devam eder.
//
// Kullanım:
//   npx tsx scripts/backfill-budget-constraint.ts --dry-run
//   npx tsx scripts/backfill-budget-constraint.ts --dry-run --limit 50
//   npx tsx scripts/backfill-budget-constraint.ts
...
async function main() {
  ...
  for (const [i, verdict] of verdicts) {
    ...
    if (!verdict.budgetConstraint) { atlanan++; continue; }
    if (verdict.budgetConstraint === "EVET") evet++; else hayir++;
    if (dryRun) {
      if (verdict.budgetConstraint === "EVET") {
    ...
    await prisma.evaluationUpsell.update({
      where: { id: row.id },
      data: { budgetConstraint: verdict.budgetConstraint },
    });
  }
